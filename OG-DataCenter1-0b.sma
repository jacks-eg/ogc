...
new iOptions[33][Item], SyncHUD[5], iMaxPlayers,  gType[33][6], gExp[33][6];
...
/* MODEL DE ARMAS */
new V_AWP[]	= "models/og_ctf1-0b/weapons/v_awp.mdl" 
new P_AWP[]	= "models/og_ctf1-0b/weapons/p_awp.mdl" 
new V_AK47[]	= "models/v_ak47.mdl" 
new V_M4A1[]	= "models/v_m4a1.mdl" 
new V_M249[]	= "models/og_ctf1-0b/weapons/v_m249.mdl"
new V_DEAGLE[]	= "models/v_deagle.mdl" 

/* HALLOWEEN */
//new V_M3[] = "models/og_ctf1-0b/halloween/v_m3.mdl"
//new V_GALIL[] = "models/og_ctf1-0b/halloween/v_galil.mdl"
...
public plugin_init() {
	...
	set_task(300.0, "check_autosave", _, _, _, "b")
...
public plugin_natives() {
	register_native("og_account", "ShowMenu_Account", 1);
	register_native("og_medallas", "Show_MenuMedallas", 1);
}
...
public check_autosave()
{
	for (new i = 1; i <= iMaxPlayers; i++)
	{
		if(!is_user_connected(i) || gExp[i][HEATSHOP] <= 0)
			return;
			
		if(containi(iSteamID[i], "STEAM_0:") != -1)
		{
			_loading(i, SAVE_STEAMID);
			player_print(i, "^3 -->^4 AUTO GUARDADO:^1 STEAMID^3 <--");
			player_print(i, "^3 -->^4 AUTO GUARDADO^3 <--^1 Tus datos han sido Actualizados...");
		} 
		else 
		{
			_loading(i, SAVE_IDLAN);
			player_print(i, "^3 -->^4 AUTO GUARDADO:^1 IDLAN^3 <--");
			player_print(i, "^3 -->^4 AUTO GUARDADO^3 <--^1 Tus datos han sido Actualizados...");
		}
	}
}
...
public ShowMenu_Account(qIndex)
{
	...
	len += formatex(menu[len], sizeof menu - 1 - len, "\r1.\w Ocultar HUD: %s^n", iOptions[qIndex][iHUD] ? "\ySi":"\rNo");
	
	len += formatex(menu[len], sizeof menu - 1 - len, "\r2.\y Personalizar:\w Jugador.^n");
	//len += formatex(menu[len], sizeof menu - 1 - len, "\r3.\y Usar:\w Brillo\d (\r Glow\d )^n");
	len += formatex(menu[len], sizeof menu - 1 - len, "\r3.\w Quitar Modelos de Admins y Armas: %s^n", iOptions[qIndex][iMODELS] ? "\ySi":"\rNo")
	len += formatex(menu[len], sizeof menu - 1 - len, "\r4.\w Mis Medallas.^n^n");
...
public HandMenuAccount(qIndex, qKeys, qMenu)
{
	switch(qKeys)
	{
		case 0:
		{
			if(!iOptions[qIndex][iHUD]) 
				iOptions[qIndex][iHUD] = 1, client_cmd(qIndex, "spk fvox/activated.wav")
			else
				iOptions[qIndex][iHUD] = 0, client_cmd(qIndex, "spk fvox/deactivated.wav")
		...
		case 3: Show_MenuMedallas(qIndex);
	}
...
public HUD(id)
{
	...
	{
		check_country(id)
		set_hudmessage(0, 255, 0, -1.0, 0.8, 0, 2.0, 2.0, 2.0)
		ShowSyncHudMsg(
			id,
			SyncHUD[0],
			"|| Jugador: %s || ADR: %d^n\
...
public fw_itemdeploy(ent) 
{
	...
	if((id>0) && is_user_connected(id))
	{
		...
		switch(iWpnId)
		{
			...
			case WEAPON_M3:
			{
				set_pev(id, pev_viewmodel2, V_M3)
				set_pev(id, pev_weaponmodel2, "models/p_m3.mdl")
			}
			case WEAPON_GALIL:
			{
				set_pev(id, pev_viewmodel2, V_GALIL)
				set_pev(id, pev_weaponmodel2, "models/p_galil.mdl")
			}*/
			case WEAPON_M249:
			{
				set_pev(id, pev_viewmodel2, V_M249);
...
stock CopyFile(const szFile[], const szNewFile[]) {
	log_to_file("og_historydb.log", "'%s' '%s' a '%s'", iInfoG[5], szFile, szNewFile);
	...
	if (iFile && iNewFile)
	{
		...
		log_to_file("og_historydb.log", "%s", iInfoG[7]);
	}
	else
	{
		log_to_file("og_historydb.log", "%s", iInfoG[6]);
	}
...
{
	static DataCenter[64], db1[25], db2[25], db3[25], db4[25], db5[25], db6[25], db7[25], db8[25], db9[25], db10[25];
	...
	switch(type)
	{
		case STEAMID:{
			...
			if(fvault_get_data("db_steamid", iSteamID[id], DataCenter, charsmax(DataCenter)))
			{
				...
				iOptions[id][iHUD] = 	str_to_num(db3)
				iOptions[id][iMODELS] =	str_to_num(db4)
				gExp[id][KNIFE] = 	str_to_num(db5)
				gType[id][KNIFE] = 	str_to_num(db6)
				gExp[id][GRENADE] = 	str_to_num(db7)
				gType[id][GRENADE] = 	str_to_num(db8)
				gExp[id][ADMIN] = 	str_to_num(db9)
				gType[id][ADMIN] = 	str_to_num(db10)
			}
		...
		case SAVE_STEAMID:{
			...
			formatex(DataCenter, charsmax(DataCenter), "%d %d %d %d %d %d %d %d %d %d", 
			gExp[id][HEATSHOP], gType[id][HEATSHOP], 
			iOptions[id][iHUD], iOptions[id][iMODELS], 
			gExp[id][KNIFE], gType[id][KNIFE], 
			gExp[id][GRENADE], gType[id][GRENADE], 
			gExp[id][ADMIN], gType[id][ADMIN])
...
public hook_teamsay(id)
{
	...
	{
		case 1: copy(playerTeamName, 11, "ROJO")
		case 2: copy(playerTeamName, 18, "AZUL")

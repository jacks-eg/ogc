...
public vote_setupEnd()
{
	...
	{
		formatex(nextMap, sizeof(nextMap)-1, "^4[^3ningun mapa^4]");
	}
	else
	{
...
{
	player_print(id, "Puedes usar^4 'rtv'^1 en vez de^4 'rockthevote'^1, si lo prefieres.");
	vote_rock(id);
...
{
	player_print(id, "Puedes usar^4 'noms'^1 en vez de^4 nominations'^1, si lo prefieres.");
	nomination_list(id);
	return PLUGIN_HANDLED;
}
...
public cmd_nextmap(id)
{
	...
	player_print(0, "Proximo Mapa:^4 %s", map);
	return PLUGIN_HANDLED;
}

public cmd_currentmap(id)
{
	player_print(0, "Mapa actual:^4 %s", g_currentMap);
	return PLUGIN_HANDLED;
}
...
public cmd_listrecent(id)
{
	switch (get_pcvar_num(cvar_banRecentStyle))
	{
		case 1:
		{
			...
			player_print(0, "Mapas recientes:^4 %s", msg[2]);	
		}
		case 2:
		{
			for (new idx = 0; idx < g_cntRecentMap; ++idx)
			{
				player_print(0, "Mapas reciente (%i):^4 %s", idx+1, g_recentMap[idx]);
			}
		}
	}
...
public cmd_startVote(id, level, cid)
{
	...
	{
		player_print(id, "Una votacion ya esta en progreso...");
	}
	else if (g_voteStatus & VOTE_IS_OVER)
	{
		player_print(id, "La votacion ha terminado.");
	}
	else
	{
...
public cmd_ff()
{
	player_print(0, "^1 Fuego amigo:^4 %L", LANG_PLAYER, get_cvar_num("mp_friendlyfire") ? "ON" : "OFF");
...
public map_manageEnd()
{
	...
	else
	{
		if (get_pcvar_num(cvar_endOnRound) && g_wasLastRound == false)
		{
			...
			{
				player_print(0, "El cambio de mapa se efectura en la proxima ronda.");
			}
			else
			{
				player_print(0, "El tiempo limite ha expirado.");
				player_print(0, "El cambio de mapa se efectura en la proxima ronda.");
			}
		...
		get_cvar_string("amx_nextmap", map, sizeof(map)-1);
		player_print(0, "El proximo mapa sera:^4 %s", map);
	}
...
nomination_attempt(id, nomination[]) // (playerName[], &phraseIdx, matchingSegment[])
{
	...
	switch (matchCnt)
	{
		case 0:
		{
			...
			player_print(id, "Nominacion fallida; ningun mapa coincide con:^4 %s", nomination);
		}		
		case 1:
		{
		...
		default:
		{
			...
			player_print(id, "Mas de un mapa coincidio con:^4 %s^1, Por favor, elija el correcto.", nomination);
			if (matchCnt == MAX_NOM_MATCH_CNT)
			{
				player_print(id, "Pueden haber mas de^4 %i^1 rondas. Solo la primera^4 %i^1 se mostrara.", MAX_NOM_MATCH_CNT, MAX_NOM_MATCH_CNT);
			}
...
nomination_cancel(id, idxMap)
{
	...
	if (g_voteStatus & VOTE_IN_PROGRESS)
	{
		player_print(id, "Cancelacion fallida; una votacion esta en progreso.");
		return;
	}
	// and if the outcome of the vote hasn't already been determined
	else if (g_voteStatus & VOTE_IS_OVER)
	{
		player_print(id, "Cancelacion fallida; el resultado de la votacion ya se ha determinado.");
		return;
	}
	...
	else
	{
		...
		if (idNominator)
		{
			...
			player_print(id, "Cancelacion fallida;^4 %s^1 fue nominado por^4 %s.", mapName, name);
		}
		else
		{
			player_print(id, "Cancelacion fallida; no haz nominado previamente^4 %s", mapName);
		}
	}
}
...
map_nominate(id, idxMap, idNominator = -1)
{
	...
	if (g_voteStatus & VOTE_IN_PROGRESS)
	{
		player_print(id, "La nominación fallo; La votacion ya esta en progreso.");
		return PLUGIN_HANDLED;
	}
	// and if the outcome of the vote hasn't already been determined
	else if (g_voteStatus & VOTE_IS_OVER)
	{
		player_print(id, "Nominacion fallida el resultado de la votacion ya fue determinado.");
		return PLUGIN_HANDLED;
	}
	...
	{
		player_print(id, "Nominacion fallida;^4 %s^1 es el mapa que esta ahora.", g_currentMap);
		return PLUGIN_HANDLED;
	}
	...
	{
		player_print(id, "Nominacion fallida;^4 %s^1 jugado muy recientemente.", mapName);
		player_print(id, "Puedes escribir,^4 'recentmaps'^1, para ver los mapas mas recientes jugados.");
		return PLUGIN_HANDLED;
	}
	...
	if (idNominator == 0)
	{
		...
		if (nominationCnt == playerNominationMax)
		{
			...
			player_print(id, "Nominacion fallida; ya haz nominado los mapas:^4 %i^1 (^4 %s^1 )", playerNominationMax, nominatedMaps);
			player_print(id, "Para nominar un mapa diferente primero debes cancelar una de tus corrientes nominaciones.");
		}
		else
		{
			...
			player_print(id, "Para cancelar tu nominacion escribe^4 'mapname' o^4 'cancel <mapname>'");
		}		
	}
	else if (idNominator == id)
	{
		player_print(id, "Nominacion fallida; ya haz nominado: %s", mapName);
	}
	else
	{
		new name[32];
		...
		player_print(id, "Nominacion fallida;^4 %s^1 ya fue nominado por^4 %s.", mapName, name);
		player_print(id, "Puedes escribir,^4 'nominations'^1, para ver una lista de las nominaciones que se estan efectuando.");
	}
	return PLUGIN_HANDLED;
}
	...
	for (new idPlayer = 1; idPlayer <= MAX_PLAYER_CNT; ++idPlayer)
	{
		for (idxNomination = 1; idxNomination <= playerNominationMax; ++idxNomination)
		{
			...
			if (idxMap >= 0)
			{
				...
				{
					player_print(0, "NOMINACIONES:^4 %s", msg[2]);
					mapCnt = 0;
					msg[0] = 0;
				}
...
//				format(hudMessage, sizeof(hudMessage)-1, "%s^n%s", hudMessage, mapName);
	...
	if (msg[0])
	{
		player_print(0, "^1 NOMINACIONES:^4 %s", msg[2]);
	}
	else
	{
		player_print(0, "^1 NOMINACIONES:^4 %L", LANG_PLAYER, "NONE");
	}
...
public vote_startDirector(bool:forced)
{
	...
	{
		player_print(0, "Creacion de Votacion Fallida; no se encontraron mapas.");
	}
	if (get_realplayersnum())
	{
		dbg_log(4, "");
	...
	if (firstPlaceVoteCnt)
	{
		...
		if (get_pcvar_num(cvar_runoffEnabled) && !(g_voteStatus & VOTE_IS_RUNOFF))
		{
			...
			if (firstPlaceVoteCnt <= totalVotes / 2)
			{
				...
				player_print(0, "Se necesita otra votacion porque no se ha recibido mas^4 50%%^1 de los votos.");
				...
				if (firstPlaceCnt > 2)
				{
					...
					player_print(0, "^4%i^1 opciones fueron empatadas para el primero. Dos de ellos fueron seleccionados al azar para la votacion.", firstPlaceCnt);
				}
				...
				else
				{
					...
					player_print(0, "La eleccion del primer lugar y una seleccion aleatoria, de^4 %i^1, la eleccion del segundo lugar estara en la votacion.", secondPlaceCnt);
				}
...
*/
		...
		if (firstPlaceCnt > 1)
		{
			...
			player_print(0, "La eleccion ganadora fue aleatoriamente seleccionada desde el^4 %i", firstPlaceCnt);
		}
		else
		{
		...
		if (idxWinner == g_choiceCnt)
		{
			...
			{
				// "stay here" won
				player_print(0, "Vamos a permanecer aqui.");
			...
			{
				// "extend map" won
				player_print(0, "El mapa actual va a ser extendido por^4 %i^1 minutos.", floatround(get_pcvar_float(cvar_extendmapStep)));
				map_extend();
			}
		}
		else 
		{
			...
			player_print(0, "El proximo mapa sera^4 '%s'", g_mapChoice[idxWinner]);
	...
	else
	{
		...
		player_print(0, "Nadie voto. El proximo mapa va a ser aleatoriamente elegido para ser:^4 '%s'", g_mapChoice[idxWinner]);
...
public vote_handleChoice(id, key)
{
	...
	if (g_voted[id] == false)
	{
		...
		if (key == 9)
		{
			...
			{
				player_print(id, "Haz elegido no ser parte de la votacion.");
			}
		}
		else
		{
			...
			if (key == g_choiceCnt)
			{
				...
				if (g_voted[id] == false)
				{
					...
					{
						player_print(id, "Haz elegido extender el mapa");
					}
				}
			}
			else
			{
				...
				{
					player_print(id, "Haz elegido:^4 %s", g_mapChoice[key]);
				}
			}
...
public vote_rock(id)
{
	...
	if (g_voteStatus & VOTE_IS_EARLY)
	{
		player_print(id, "Hay una votacion ya programada para que tenga lugar en menos de 2 minutos.");
		return PLUGIN_HANDLED;
	}
	...
	if (g_rtvWait)
	{
		...
		{
			player_print(id, "Debes esperar aproximadamente^4 %i^1 minutos antes de que puedas dar un^4 Rock The Vote^4 (RTV)", floatround(g_rtvWait - minutesElapsed, floatround_ceil));
			return PLUGIN_HANDLED;
		}
	}
	...
	if (g_voteStatus & VOTE_IN_PROGRESS)
	{
		player_print(id, "Una votacion esta ya en progreso.");
		return PLUGIN_HANDLED;
	}
	// and if the outcome of the vote hasn't already been determined
	else if (g_voteStatus & VOTE_IS_OVER)
	{
		player_print(id, "El resultado de la votacion ya fue determinado.");
		return PLUGIN_HANDLED;
	}
	...
	if (g_rockedVote[id])
	{
		player_print(id, "Ya haz votado en la votacion.", rocksNeeded - g_rockedVoteCnt);
	...
	player_print(id, "Haz hecho el^4 (RTV)");
	...
	if (++g_rockedVoteCnt >= rocksNeeded)
	{
		...
		client_print(0, print_chat, "^1 Suficientes jugadores dado^4 rtv^1, para la votacion del proximo mapa, la votacion va empezar.");
...
public rtv_remind(param)
{
	...
	// let the players know how many more rocks are needed
	player_print(who, "^4%i^1 mas jugadores quieren hacer un^4 'RTV'^1, para iniciar la votacion.", vote_getRocksNeeded() - g_rockedVoteCnt);
}
...
{
	player_print(0, "Los siguientes mapas no fueron todavia nominados:^4 %s", nominations);
}
...
map_announceNomination(id, map[])
{
	...
	player_print(0, "^4%s^1 fue nominado^4 '%s'", name, map);
}
...
{	
	...
	if (mode & dbg)
	{
		...
		// log text to file
		log_to_file("galileo.log", "{%3.4f} %s", gameTime, formattedText);

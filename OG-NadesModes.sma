...
public event_armnade(id)
{
	...
	if (get_option(OPTION_DISPLAY_MODE_ON_DRAW) && grenade_can_be_used(NadeRace:nade))
	{
		...
		new Temp[64];
		format(Temp, charsmax(Temp), "|| Modo: %s ||", modetext[_:mode[id][NadeRace:nade]])
		//enviarHint(id, Temp, 8, 3.0);
		center_msj(id, 4, Temp);
	}
...
public changemode(id, NadeRace:NADE_TYPE)
{
	...
	switch (mode[id][NADE_TYPE])
	{
		...
		{
			//""enviarHint(id, "", 8, 1.0);
			center_msj(id, 4, "|| Modo:  Normal ||");
		}
		
		case NADE_PROXIMITY:
		{
			//enviarHint(id, "|| Modo:  Proximity ||", 8, 1.0);
			center_msj(id, 4, "|| Modo:  Proximity ||");
		}
		
		case NADE_IMPACT:
		{
			//enviarHint(id, "|| Modo: Impact ||", 8, 1.0);
			center_msj(id, 4, "|| Modo: Impact ||");
		}
		
		case NADE_TRIP:
		{
			//enviarHint(id, "|| Modo: Trip laser ||", 8, 1.0);
			center_msj(id, 4, "|| Modo: Trip laser ||");
		}
		
		case NADE_MOTION:
		{
			//enviarHint(id, "|| Modo: Motion sensor ||", 8, 1.0);
			center_msj(id, 4, "|| Modo: Motion sensor ||");
		}
		
		case NADE_SATCHEL:
		{
			//enviarHint(id, "|| Modo: Satchel charge ||", 8, 1.0);
			center_msj(id, 4, "|| Modo: Satchel charge ||");
		}
		
		case NADE_HOMING:
		{
			//enviarHint(id, "|| Modo: Homing ||", 8, 1.0);
			center_msj(id, 4, "|| Modo: Homing ||");
		}

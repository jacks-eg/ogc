...
#define MAXSLOTS 32
new const TAG[] = "^3[^1AstroStrike^3]^1";
...
public SayStatsAll          = 1 // displays all players stats and rank
...
// iAwardID
new iTopX, iTopEnd, iDesign, g_Vault, g_Vault2

new pcvar_statsmarquee, pcvar_statsmarquee_effect, pcvar_statsmarquee_color, pcvar_statsmarquee_position
new pcvar_award,  pcvar_award_anonce_chat, pcvar_award_chance, pcvar_award_cash, pcvar_hs, //pcvar_bot, pcvar_dmg
pcvar_topx
...
new const gThemeOG[] = 
{
	"<meta charset=UTF-8>\
	<style>\
	body{background:url('http://74.91.127.79:8880/stats-astro/bg.jpg');background-size: 100%%;font-family:Arial;text-align: center;color:#FFF}\
	table{font-size:12px;}\
	th{background:#008aff url('https://global.gaming.site.nfoservers.com/images/bg.png');padding: 10px 2px;text-align:left}\
	td{background-color:rgba(0,0,0,0.5);padding: 5px}\
	.top1{color:#e8c021;text-shadow: 0 0 0.9em #ffbf00;font-weight: bold;}\
	.top2{color:#008aff;text-shadow: 0 0 0.9em #008aff;font-weight: bold;}\
	.top3{color:#bb0c11;text-shadow: 0 0 0.9em #bb0c11;font-weight: bold;}\
	</style>"
...
new const g_szCups[ ][ ] = {
    "http://74.91.127.79:8880/stats-astro/top1.png",
    "http://74.91.127.79:8880/stats-astro/top2.png",
    "http://74.91.127.79:8880/stats-astro/top3.png"
}; 
...
public plugin_init()
{
	...
	register_dictionary("stats_shell.txt")
	...
	register_clcmd("say evento", "cmdWiners");
...
public plugin_cfg()
{
	...
	formatex(szCfgDir, sizeof (szCfgDir) -1 , "%s/stats_shell/config.cfg", szCfgDir)
...
format_topx(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><p><img src='http://74.91.127.79:8880/stats-astro/top.png' width=70%%></p><table width=100%%")
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><tr><th width=10><th width=100>Jugadores<th>Asesinatos<th>Muertes</tr>")
...
format_place1(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><p><img src='http://74.91.127.79:8880/stats-astro/top10.png' width=70%%></p><table width=100%% border=0 cellpadding=0 cellspacing=0")
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><tr><th width=10>#<th width=100>Jugadores<th>Asesinatos<th>Muertes</tr>")
...
format_winers(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	new iMax = get_statsnum()
	new izStats[8], izBody[8]
	new iLen = 0
	...
	iLen = format(sBuffer, MAX_BUFFER_LENGTH, gThemeOG)
	
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,"<body><p><img src='http://74.91.127.79:8880/stats-astro/top.png' width=70%%></p>");
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen,"<table width=100%% border=0 cellpadding=0 cellspacing=0<tr><th width=10><th width=200>Ganadores Actuales<th>Asesinatos<th>Muertes</tr>")
	...
	for (new i = 0; i < iMax && MAX_BUFFER_LENGTH - iLen > 0; i++)
	{
		...
		if( i < iMaxColors )
		{
			iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<tr class='%s'><td><img src=^"%s^"/><td>%s<td>%d<td>%d", g_szTopColors[i], g_szCups[i], t_sName, izStats[STATS_KILLS], izStats[STATS_DEATHS])	
		}
		else 
		{	
			iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<tr><td>%d<td>%s<td>%d<td>%d", i + 1, t_sName, izStats[STATS_KILLS], izStats[STATS_DEATHS])
		}
	}
}
...
format_place2(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><p><img src='http://74.91.127.79:8880/stats-astro/top20.png' width=70%%></p><table width=100%% border=0 cellpadding=0 cellspacing=0")
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><tr><th width=10><th width=100>Jugadores<th>Asesinatos<th>Muertes</tr>")
...
format_place3(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><p><img src='http://74.91.127.79:8880/stats-astro/top30.png' width=70%%></p><table width=100%% border=0 cellpadding=0 cellspacing=0")
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body><tr><th width=10><th width=100>Jugadores<th>Asesinatos<th>Muertes</tr>")
...
public cmdStatsMe(id)
{
	if (!SayStatsMe)
	{
		player_print(id, "^4 %L", id, "DISABLED_MSG")
		return PLUGIN_HANDLED
	}
...
public cmdWiners(id)
{
	...
	format_winers(g_sBuffer)
	show_motd(id, g_sBuffer, "|| Ganadores Actuales ||")
...
public cmdRank(id)
{
	...
	player_print(id, "Tu Ranking es:^4 %d^1 /^4 %d^1 ||^4 %d^3 Asesinatos^1 ||^4 %d^3 Muertes^1 ||^4 %d^3 Exito(s)^1 ||^3 Nivel de Habilidad:^4 %0.0f%%", iRankPos, iRankMax, izStats[STATS_KILLS], izStats[STATS_DEATHS], izStats[STATS_HITS], ((effec(izStats) + accuracy(izStats)) / 2))
...
public cmdScore(id)
{
	...
	player_print(id, "^1 %L: %s", id, "GAME_SCORE", g_sScore)
...
public cmdSwitch(id)
{
	...
	player_print(id, "^4 * %L", id, "STATS_ANNOUNCE", lEnDis)
		...
		{ 
			...
			if(equal(szName, t_sName))
			{
				...
				if(get_pcvar_num(pcvar_award_anonce_chat))
				{
					...
					{
						player_print(szPlayers[k], "^1 %L", LANG_SERVER, "MM_AWARD", iAwardCash, iAwardCash_Bonus, i + 1)
					}
				}
			}
		} 
...
public time_play_bonus()
{
	...
	for(new i = 0; i < iNum; i++)
	{
		...
		player_print(szPlayers[i], "^4 %L", LANG_SERVER, "MM_TIME_PLAY_BONUS", iTime, iTmpCash)

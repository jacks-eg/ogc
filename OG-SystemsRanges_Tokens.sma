...
public DataHandler(failstate, error[], error2, data[], size, Float:queuetime){
	...
	switch( data[ 1 ] ) {
		...
		case AUTO_LOGIN_ADD: {
			if( mysql_num_results() ) {
				client_printc( id, "Comprobacion de^4 Usuario^1 y^4 Contraseñ^1 completada..." );
				
				new szQuery[ 256 ], iData[ 2 ];
	
				iData[ 0 ] = id;
				iData[ 1 ] = AUTO_LOGIN_INSET;
		
				formatex( szQuery, charsmax( szQuery ), "INSERT INTO %s (user, password, steamid) VALUES (^"%s^", ^"%s^", ^"%s^")", MYSQL_TABLE2, g_szHashUser[ id ], g_szHashPass[ id ], g_steamid[ id ] );
				mysql_query(g_hTuple, "DataHandler", szQuery, iData, 2);
				
			}
			else {
				client_printc( id, "^4Usuario^1 o^4 Contraseñ^1 no se encuentran registrados..." );
				client_cmd( id, "spk buttons/button10.wav" );
				_Show_account( id );
			}
		}
		case AUTO_LOGIN_INSET: {
		
			if( failstate < TQUERY_SUCCESS ) {
				if( containi( error, "user" ) != -1 )
					client_printc( id,"el^4 usuario^1 se encuentra en uso..." );
				
				else if( containi( error, "steamid" ) != -1 )
					client_printc( id,"la^4 steamid^4 ya a sido usada en otra cuenta..." );
				else
					client_printc( id,"Error al agregar el autologin. Intente nuevamente." );
				
				client_cmd( id, "spk buttons/button10.wav" );
				_Show_account( id );
			}
			else {
				client_printc( id, "Felicidades!!^4 Autologin^1 activo con exito..." );
				client_printc( id, "Datos:^3 Cuenta^1 -^4 %s^1 ||^3 SteamID:^4 %s", g_usuario[ id ], g_steamid[ id ] );
				_Show_account( id );
			}
		}
		case AUTO_LOGIN_CHECKER: {
			if( mysql_num_results() ) {
				new szQuery[ 256 ], iData[ 2 ];
				new iAuto[33], iAuto2[33];
				
				mysql_read_result(2, iAuto, charsmax(iAuto));
				mysql_read_result(3,iAuto2, charsmax(iAuto2));

				iData[ 0 ] = id;
				iData[ 1 ] = LOGUEAR_CUENTA;
	
				formatex( szQuery, charsmax( szQuery ), "SELECT * FROM %s WHERE Usuario=^"%s^" AND Password=^"%s^"", MYSQL_TABLE, iAuto, iAuto2 );
				mysql_query( g_hTuple, "DataHandler", szQuery, iData, 2 );
			} else {
				client_printc( id, "Usuario o Contraseñ incorrecta." );
				client_cmd( id, "spk buttons/button10.wav" );
...
public funct_account(id, menu, item) {
	...
	switch(item){
		case 0: client_printc(id, "Opcion:^4 1");
		case 1: {
